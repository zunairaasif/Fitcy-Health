...
import ConversationsMobile from "./ConversationMobile";
...
const PatientSelector = ({
  ...
  const patients = useSelector((state) => state.patient.patientTherapists);
  ...
  if (searchVal && filteredPatients)
    filteredPatients = filteredPatients.filter(
      (x) =>
        x.therapist.first_name
          .toLowerCase()
          .includes(searchVal.toLowerCase()) ||
        x.therapist.last_name.toLowerCase().includes(searchVal.toLowerCase()) ||
        x.therapist.first_name
          .toLowerCase()
          .concat(" " + x.therapist.last_name.toLowerCase())
          .includes(searchVal.toLowerCase())
    );
  ...
  useEffect(() => {
    if (!patients) dispatch(getPatientTherapists({ patient: user.id }));
...
export default function Messages() {
      ...
      try {
        ...
        rtm
          ...
          .then(() => {
            ...
            if (peerID && peerID.id) {
              rtm.joinChannel(user.id.toString());
  ...
  const loadMessageHistory = (peerid) => {
    dispatch(getMessageHistory({ therapist_id: peerid, patient_id: user.id }));
  };
